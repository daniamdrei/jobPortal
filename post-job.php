...
<?php 
    ...
    $categorie->execute();
    $get_categorie = $categorie->fetchAll(PDO::FETCH_OBJ);

    if(isset($_POST['submit'])) {

      if(empty($_POST['job_title']) OR empty($_POST['job_region']) OR empty($_POST['job_type']) OR empty($_POST['Job_Categorie']) OR empty($_POST['vacancy']) OR empty($_POST['experience']) 
      ...
      OR empty($_POST['education_experience']) OR empty($_POST['other_benifits'])
      ) {
          header('location:post-job.php?error=some input are empty!');
      } else {

        $job_title = $_POST['job_title'];
        $job_region = $_POST['job_region'];
        $job_type = $_POST['job_type'];
        $Job_Categorie = $_POST['Job_Categorie'];
        ...
        $insert = $conn->prepare("INSERT INTO jobs (job_title, job_region, job_type, Job_Categorie, vacancy, experience, salary, gender, application_deadline,
         job_description, responsibilities, education_experience, other_benifits, company_email, company_name, company_id, company_image) VALUES(
          :job_title, :job_region, :job_type, :Job_Categorie ,:vacancy, :experience, :salary, :gender, :application_deadline,
        ...
        <div class="row mb-5">
          <div class="col-lg-12">
            ...
            <?php if(isset($_GET['error'])){ ?>
              ...
              <div class="form-group">
                <label for="job-type"> Job Categorie</label>
                <select name="Job_Categorie" class="selectpicker border rounded" id="job-type" data-style="btn-black" data-width="100%" data-live-search="true" title="Select Job Categorie">
                  <?php foreach($get_categorie as $category ): ?>
              ...
              <div class="form-group">
                <input type="hidden" value="<?php echo $_SESSION['user_email']; ?>"name="company_email" class="form-control" id="" placeholder="Company Email">
              </div>
              <div class="form-group">
                <input type="hidden" name="company_name" value="<?php echo $_SESSION['user_name']; ?>" class="form-control" id="" placeholder="Company Name">
              </div>
              <div class="form-group">
                <input type="hidden" name="company_id" value="<?php echo $_SESSION['user_id']; ?>" class="form-control" id="" placeholder="Company ID">
              </div>
              <div class="form-group">
                <input type="hidden" name="company_image" value="<?php echo $_SESSION['user_img']; ?>" class="form-control" id="" placeholder="Company Image">
              </div>
